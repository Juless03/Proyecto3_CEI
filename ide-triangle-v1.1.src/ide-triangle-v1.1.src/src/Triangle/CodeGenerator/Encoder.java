...
import TAM.Machine;
import static TAM.Machine.succDisplacement;
...
public Object visitForCommand(ForCommand ast, Object o) {
    ...
    // Guarda el inicio del bucle
    loopAddr = nextInstrAddr;

    // Revisa la condición: 'I <= Expression2'
    
    ast.E1.visit(this, frame);
    condAddr = nextInstrAddr;
    ast.E2.visit(this, frame);
    emit(Machine.JUMPIFop, Machine.falseRep, Machine.CBr, nextInstrAddr);

    // Ejecuta el bucle (cuerpo)
    ast.C.visit(this, frame);

    // Incrementa 'I'
    ast.I.visit(this, frame);
    emit(Machine.LOADLop, 0, 0, 1); // carga 1 en el stack
    emit(Machine.CALLop, Machine.SBr, Machine.PBr, Machine.addDisplacement);

    // Revisa la condición de nuevo y salta al inicio del bucle si es verdadera
    ast.I.visit(this, frame);
    emit(Machine.JUMPIFop, Machine.trueRep, Machine.CBr, loopAddr);

    //Guarda la direccion para el siguiente salto después de la condición
    afterLoopAddr = nextInstrAddr;
    patch(condAddr, afterLoopAddr);
...
public Object visitCaseCommand(CaseCommand ast, Object o) {
    Frame frame = (Frame) o;
    int jumpAddr, endAddr;
    Map<IntegerLiteral, Integer> jumpTable = new HashMap<>();

    ast.E.visit(this, frame);

    for (IntegerLiteral il : ast.CC.keySet()) {
        jumpAddr = nextInstrAddr;
        jumpTable.put(il, jumpAddr);
        emit(Machine.JUMPop, 0, Machine.CBr, 0);
    }
