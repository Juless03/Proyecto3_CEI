...
// to represent its phrase structure.
    ...
    Command parseSingleCommand() throws SyntaxError {
        ...
        switch (currentToken.kind) {
            ...
            case Token.FOR: {
                acceptIt();
                Identifier iAST = parseIdentifier(); // Variable del bucle
                accept(Token.FROM);
                Expression e1AST = parseExpression(); // Expresión inicial
                accept(Token.TO);
                Expression e2AST = parseExpression(); // Expresión final
                accept(Token.DO);
                Command cAST = parseSingleCommand(); // Cuerpo del bucle
                finish(commandPos);
            ...
            case Token.CASE: {
                ...
                //ArrayList<IntegerLiteral> caseCommands = new ArrayList<>();
                HashMap<IntegerLiteral, Command> caseCommands = new HashMap<>();
                ...
                while (currentToken.kind != Token.ELSE) {
                    ...
                    caseCommands.put(iAST, cAST);
                }

                accept(Token.ELSE);
                Command c1AST = parseCommand();
                finish(commandPos);
                commandAST = new CaseCommand(eAST, caseCommands, c1AST, commandPos);
