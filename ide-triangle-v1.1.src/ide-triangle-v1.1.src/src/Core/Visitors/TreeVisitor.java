...
public Object visitCaseCommand(CaseCommand ast, Object obj) {
    Object exprResult = ast.E.visit(this, obj);
    ArrayList<DefaultMutableTreeNode> casesResults = new ArrayList<>();
    
    for(Map.Entry<IntegerLiteral, Command> entry : ast.CC.entrySet()){
        AST keyResult = (AST) entry.getKey().visit(this, obj);
        AST valueResult = (AST) entry.getValue().visit(this, obj);
        casesResults.add(createBinary("Case Entry", keyResult, valueResult));
    }
